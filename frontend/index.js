...
function Dashboard() {
  ...
  const selectedProjectObj = React.useMemo(() => {
    ...
    const projeto = lista_projetos.find(
      (proj) => proj.nome === selectedProject
    );
  ...
  const score_recalc_consultores = (scores || []).map((membro) => {
    ...
    if (dispon === 0) {
      nota += 3 * pesos.availability;
    } else if (dispon === 1) {
      nota += 2 * pesos.availability;
    } else if (dispon === 2) {
      nota += 1 * pesos.availability;
    }
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="inline-block text-[14px] font-semibold px-3 py-[6px] rounded-full text-[var(--primary,#3b82f6)] bg-[color-mix(in_srgb,var(--primary,#3b82f6)_12%,transparent)]">
                  {overallScore >= 8 ? (
                    ...
                    <span className="flex items-center gap-1">
                      <ClipboardEdit size={14} className="shrink-0" /> A
                      Melhorar
                    </span>
                  )}
                </div>
